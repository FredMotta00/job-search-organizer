...
export default async function JobsPage({ searchParams }: { searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  return <>
    <PageHeader eyebrow="Oportunidades" title="Vagas" description="Vagas manuais e alertas oficiais do LinkedIn, Indeed e Glassdoor em uma única fila." />
    ...
    {query.descobertas !== undefined && <div className="notice success">Busca concluída: {query.descobertas} nova(s), {query.fila ?? 0} preparada(s) para revisão e {query.arquivadas ?? 0} arquivada(s) pela nota mínima.</div>}
    ...
    <Card style={{marginTop:18} as React.CSSProperties}><h2>Pipeline</h2>{filtered.length === 0 ? <Empty title="Nenhuma vaga encontrada">Cadastre uma oportunidade abaixo ou ajuste os filtros.</Empty> : <div className="list">{filtered.map(job => { const score=job.discoveryScore ?? job.manualScore ?? job.assessment?.score; return <Link className="listItem" key={job.id} href={`/vagas/${job.id}`}><div><div className="chips"><Badge>{job.source}</Badge>{job.discoveryScore!==null&&<Badge tone="info">descoberta automática</Badge>}{job.incomplete&&<Badge tone="warn">incompleta</Badge>}{job.possibleDuplicate&&<Badge tone="warn">possível duplicata</Badge>}</div><strong>{job.title}</strong><p>{job.company}{job.location?` · ${job.location}`:""}</p><small>{STATUS_LABELS[job.status]}</small></div><Badge tone={(score??0)>=80?"good":(score??0)>=65?"info":"neutral"}>{score ?? "—"}/100</Badge></Link>})}</div>}</Card>
