...
import { choosePreferredResume, jobLooksEnglish } from "@/lib/application-fields";
...
export default async function JobDetail({ params, searchParams }: { params: Promise<{id:string}>; searchParams: Promise<Record<string,string|undefined>> }) {
  ...
  const recommendedResume=choosePreferredResume(fileResumes,job);
  return <>
    ...
    <div className="grid two">
      ...
      <Card><h2>Status e histórico</h2><form action={transitionJob} className="stack"><input type="hidden" name="jobId" value={id}/><label>Próximo estado<select name="toStatus" required>{allowedTransitions(job.status).map(s=><option value={s} key={s}>{STATUS_LABELS[s]}</option>)}</select></label><label>Anotação<textarea name="note" /></label><label>Evidência do envio (opcional)<input name="evidence" placeholder="Confirmação ou identificador" /></label><label>Currículo usado<select name="resumeVersionId" defaultValue={recommendedResume?.id??""}><option value="">Não informado</option>{resumes.map(r=><option key={r.id} value={r.id}>{r.name}{r.isDefault?" — padrão":""}</option>)}</select></label><label className="checkbox"><input type="checkbox" name="confirmedSent"/> Confirmo que o envio foi concluído</label><button disabled={allowedTransitions(job.status).length===0}>Registrar transição</button></form>
    ...
    <div className="grid two" style={{marginTop:18}}>
      <Card><h2>Dados do anúncio</h2><div className="chips"><Badge>{job.workMode||"modelo desconhecido"}</Badge><Badge>{job.seniority||"senioridade desconhecida"}</Badge><Badge>{job.contractType||"contratação desconhecida"}</Badge>{job.incomplete&&<Badge tone="warn">incompleta</Badge>}</div>{job.discoveryScore!==null&&<div className="callout"><strong>Relevância da descoberta: {job.discoveryScore}/100</strong><p>{job.discoveryReason}</p></div>}<pre className="textPreview">{job.description||"Descrição não informada."}</pre></Card>
    ...
    <Card style={{marginTop:18} as React.CSSProperties}><h2>Preparação da candidatura</h2><p className="muted">Os textos usam apenas fatos explicitamente confirmados. O currículo marcado como padrão já vem selecionado para todas as vagas.</p><form action={prepareApplication} className="actions"><input type="hidden" name="jobId" value={id}/><select name="resumeId" style={{maxWidth:360}} defaultValue={recommendedResume?.id??""}><option value="">Sem currículo selecionado</option>{resumes.map(r=><option key={r.id} value={r.id}>{r.name}{r.isDefault?" — padrão":""}</option>)}</select><button>Gerar materiais determinísticos</button></form>
    ...
    <Card style={{marginTop:18} as React.CSSProperties}>
      ...
      <form action={startSemiAutomaticApplication} className="stack automationForm">
        ...
        <label>Currículo para anexar<select name="resumeId" defaultValue={recommendedResume?.id ?? ""} required><option value="" disabled>Selecione um currículo com arquivo</option>{fileResumes.map(resume=><option key={resume.id} value={resume.id}>{resume.name}{resume.isDefault?" — padrão para todas as vagas":resume.id===recommendedResume?.id?` — recomendado para vaga em ${englishJob?"inglês":"português"}`:""}</option>)}</select></label>
